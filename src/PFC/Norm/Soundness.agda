...
collect-fun (elem (Δ , nil , t0))      = return t0
collect-fun (elem (Δ , single n , t0)) = letin (embNe .apply n) t0

collect-pres-≋ : Pres-≋ (◇' Tm'- a) (Tm'- (◇ a)) collect-fun
collect-pres-≋ {p = elem (_ , nil , _)}      (proof (≡-refl , ≡-refl , t≈t'))
  = cong-return t≈t'
collect-pres-≋ {p = elem (_ , single _ , _)} (proof (≡-refl , ≡-refl , t≈t'))
  = cong-letin2 t≈t'

collect-nat : Natural (◇' Tm'- a) (Tm'- (◇ a)) collect-fun
collect-nat w (elem (Δ , nil , t0))      = ≈-refl
collect-nat w (elem (Δ , single n , t0)) = cong-letin1 (embNe .natural w n)
...
collect-comm = record { proof = λ
    { (elem (_ , nil , n0))      → ≈-refl
    ; (elem (_ , single x , n0)) → ≈-refl
    }
  }
...
... | elem (Θt , nil , x) | (t' , tr-t , t'Lx)
  = substTm (wkSub ⊆-refl s `, t') u
  , ≈-trans (cong-letin1 tr-t) (red-dia2-tr-lemma s t' u)
  , fund u (wkSub-pres-ℒₛ ⊆-refl sLδ , t'Lx)
... | elem (Θt , single n , x) | (t' , tr-t , t'Lx)
  = substTm (wkSub freshWk s `, t') u
  , ≈-trans (cong-letin1 tr-t) (red-dia1-tr-lemma s (embNe .apply n) t' u)
