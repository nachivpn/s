...
module PFC.Term.Conversion where

open import PFC.Term.Base
open import PFC.Term.Properties
...
data _≈_ : Tm Γ a → Tm Γ a → Set where
  ...
  red-dia2 : (t : Tm Γ a)  (u : Tm (Γ `, a) b)
    → letin (return t) u ≈ return (substTm (idₛ `, t) u)
...
wkTm-pres-≈ w (red-dia1 t u u')     = ≈-trans (red-dia1 _ _ _) (cong-letin2 (≡-to-≈ (red-dia1-crunch-lemma w t u u')))
wkTm-pres-≈ w (red-dia2 t u)        = ≈-trans (red-dia2 _ _) (cong-return (≡-to-≈ (red-fun-crunch-lemma w t u)))
...
red-dia2-tr-lemma s t u = let open EqReasoning (Tm-setoid _ _) in begin
  ...
  return (substTm (idₛ `, t) (substTm (keepₛ s) u))
    ≡˘⟨ cong return (substTm-pres-∙ₛ (keepₛ s) (idₛ `, t) u) ⟩
  return (substTm (keepₛ s ∙ₛ (idₛ `, t) ) u)
    ≡⟨⟩
  return (substTm ((dropₛ s ∙ₛ (idₛ `, t)) `, t) u)
    ≡⟨ cong (λ z → return (substTm (z `, t) u)) (red-dia2-crunch-lemma ⊆-refl s t) ⟩
  return (substTm (wkSub ⊆-refl s `, t) u)
    ∎
