...
abstract
  ...
  evalTm-sound (exp-dia {Γ} {a} t) = let open EqReasoning (Tm'-setoid Γ (◇ a)) in begin
    evalTm t
      ≈⟨ exp-dia' (evalTm t) ⟩
    letin' (evalTm t) (return' π₂')
      ≡⟨⟩
    letin' (evalTm t) (return' (evalVar {Γ `, a} zero)) ∎
  evalTm-sound (red-dia {Γ} {a} {b} t u) = let open EqReasoning (Tm'-setoid Γ (◇ b)) in begin
    letin' (return' (evalTm t)) (evalTm u)
      ≈⟨ red-dia' (evalTm t) (evalTm u) ⟩
    evalTm u [ ⟨ id' , evalTm t ⟩' ]'
      ≈˘⟨ ∘-pres-≈̇-right (evalTm u) (⟨,⟩'-pres-≈̇-left (evalSub-pres-id Γ) (evalTm t)) ⟩
    evalTm u [ ⟨ evalSub idₛ[ Γ ] , evalTm t ⟩' ]'
      ≈˘⟨ evalTm-pres-∘ u (idₛ `, t) ⟩
    evalTm (substTm (idₛ `, t) u)
      ∎
  evalTm-sound (ass-dia {Γ} {a} {b} {c} t u u') = let open EqReasoning (Tm'-setoid Γ (◇ c)) in begin
    letin' (letin' (evalTm t) (evalTm u)) (evalTm u')
      ≈⟨ ass-dia' (evalTm t) (evalTm u) (evalTm u') ⟩
    letin' (evalTm t) (letin' (evalTm u) (evalTm u' ∘ (π₁' ×'-map id')))
      ≈˘⟨ letin'-pres-≈̇-right _ (letin'-pres-≈̇-right _ (∘-pres-≈̇-right (evalTm u')
            (⟨,⟩'-pres-≈̇-left (∘-pres-≈̇-left (evalWk-pres-π₁ Γ a) _) _))) ⟩
    letin' (evalTm t) (letin' (evalTm u) (evalTm u' ∘ (evalWk freshWk[ Γ , a ] ×'-map id')))
      ≈˘⟨ letin'-pres-≈̇-right _ (letin'-pres-≈̇-right _ (∘-pres-≈̇-right (evalTm u')
            (evalWk-pres-×-map-id {a = b} {w = freshWk[ Γ , a ] }))) ⟩
    letin' (evalTm t) (letin' (evalTm u) (evalTm u' [ evalWk (keep[ b ] freshWk[ Γ , a ]) ]'))
      ≈˘⟨ letin'-pres-≈̇-right _ (letin'-pres-≈̇-right _ (evalTm-pres-∘' (keep freshWk[ Γ , a ]) u')) ⟩
    letin' (evalTm t) (letin' (evalTm u) (evalTm (wkTm (keep freshWk[ Γ , a ]) u')))
      ∎
