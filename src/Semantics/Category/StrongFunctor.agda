...
record HasStrongFunctor (C : Category) (isCartesian : IsCartesian C) : Set₂ where
  ...
  letin' {P} {Q} φ ψ = (◯'-map ψ ∘ ◯'-strength P Q) ∘ pr' id' φ

  abstract

    letin'-pres-≈̇ : ∀ {P Q R : Obj} {φ φ' : P →̇ ◯' Q} {ψ ψ' : (P ×' Q) →̇ R} → (φ≈̇φ' : φ ≈̇ φ') (ψ≈̇ψ' : ψ ≈̇ ψ') → letin' φ ψ ≈̇ letin' φ' ψ'
    letin'-pres-≈̇  {P} {Q} φ≈̇φ' ψ≈̇ψ' = ∘-pres-≈̇ (∘-pres-≈̇-left (◯'-map-pres-≈̇ ψ≈̇ψ') (◯'-strength P Q)) (⟨,⟩'-pres-≈̇-right _ φ≈̇φ')
  ...
  abstract
    ...
    ◯'-strength-π₂ {P} {Q} = let open EqReasoning (→̇-setoid (P ×' (◯' Q)) (◯' Q)) in begin
      ◯'-map π₂' ∘ ◯'-strength P Q
        ≈˘⟨ ∘-pres-≈̇-left (◯'-map-pres-≈̇ ((≈̇-trans (×'-beta-right (unit' ∘ π₁')) (id'-unit-left Q π₂')))) (◯'-strength P Q)⟩
      ...
      ◯'-map π₂' ∘ ◯'-map (unit' ×'-map id') ∘ ◯'-strength P Q
        ≈˘⟨ ∘-pres-≈̇-right (◯'-map π₂') (◯'-strength-natural₁ unit') ⟩
